...
<?php
		...
		$perpage	=	12;
		$r_all		=	$db->select("tgp_cms","hien_thi = 1 AND cat IN (".$cate_id.") ","order by time desc");
		...
		$r2	= $db->select("tgp_cms","hien_thi = 1 AND cat IN (".$cate_id.")  ","order by time desc limit $min,$max");
		while ($row2 = $db->fetch($r2))
		{
		?>
		<div class="itemnews"> 
		  <a href="<?php echo $liveSite.'/thong-bao-chi-tiet/'.$row2['id'].'/'.lg_string::get_link($row2['ten']).'/'; ?>">
		  <?php if ($row2["hinh"] != "no") {
		  ...
		  <?php	
			...
			<div class="text-itemnews">
			  <h2><a href="<?php echo $liveSite."/thong-bao-chi-tiet/".$row2["id"]."/".lg_string::get_link($row2["ten"])."\"";?>"><?php echo lg_string::crop($row2["ten"],7);?></a>
			  <?php 
...
{
	var url=root+"/danh-muc-thong-bao/"+cateId+"/"+pageNo+"/index.html";
