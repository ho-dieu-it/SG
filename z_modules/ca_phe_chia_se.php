<div class="news">
<p> Home > Cà phê chia sẻ</p>
<div class="detail">
<div class="inner-news">
<?php
		// Paging ======
		$cate_id=4;$page=isset($_GET['page'])?$_GET['page']:((isset($page))?$page:0) + 0;
		
		//$page		=	((isset($page))?$page:0) + 0;
		$perpage	=	12;
		$r_all		=	$db->select("tgp_cms","hien_thi = 1 AND cat IN ('4')  ","order by time desc");
		...
		$r2	= $db->select("tgp_cms","hien_thi = 1 AND cat IN ('4')  ","order by time desc limit $min,$max");
		while ($row2 = $db->fetch($r2))
		{
		?>
		<div class="itemnews"> 
		  <a href="<?php echo $liveSite.'/ca-phe-chia-se-chi-tiet/'.$row2['id'].'/'.lg_string::get_link($row2['ten']).'/'; ?>">
		  <?php if ($row2["hinh"] != "no") {
		  ...
		  <?php	
			...
			<div class="text-itemnews">
			  <h2><a href="<?php echo $liveSite."/ca-phe-chia-se-chi-tiet/".$row2["id"]."/".lg_string::get_link($row2["ten"])."\"";?>"><?php echo lg_string::crop($row2["ten"],7);?></a>
			   <?php 
...
{
	var url=root+"/ca-phe-chia-se/"+cateId+"/"+pageNo+"/index.html";
